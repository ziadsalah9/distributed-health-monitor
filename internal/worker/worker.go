...
func processCheck(id uint, hub *websocket.Hub) {
	...
	if err == nil {
		...
		status = EvaluateStatus(resp.StatusCode)
	...
	if HasStateChanged(service.LastStatus, status) {
...
func EvaluateStatus(statusCode int) string {
...
func HasStateChanged(oldStatus, newStatus string) bool {
